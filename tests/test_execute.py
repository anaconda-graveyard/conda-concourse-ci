...
def test_collect_tasks(mocker, testing_conda_resolve, testing_graph):
    ...
    task_graph = execute.collect_tasks(
        graph_data_dir, folders=["a"], matrix_base_dir=test_config_dir
    )
...
def test_graph_to_plan_with_jobs(mocker, testing_graph):
    ...
    pipeline = execute.graph_to_plan_with_jobs(
...
def test_submit_batch(mocker):
    ...
    get_activate_builds = mocker.patch.object(
        execute, "_get_activate_builds", return_value=3
    )
    ...
    submit_one_off.assert_has_calls(
        [
            mocker.call(
                ...
                mocker.ANY,
                pass_throughs=None,
                clobber_sections_file="example.yaml",
            ),
            mocker.call(
                "sentinel_pytest",
                mocker.ANY,
...
def test_compute_builds(testing_workdir, mocker, monkeypatch):
    ...
    assert os.path.isfile(
        os.path.join(output, "frank-1.0-python_2.7-on-centos5-64", "meta.yaml")
    )
    assert os.path.isfile(
        os.path.join(
            output, "frank-1.0-python_2.7-on-centos5-64/", "conda_build_config.yaml"
        )
    )
    assert os.path.isfile(
        os.path.join(output, "frank-1.0-python_3.6-on-centos5-64", "meta.yaml")
    )
    assert os.path.isfile(
        os.path.join(
            output, "frank-1.0-python_3.6-on-centos5-64/", "conda_build_config.yaml"
        )
    )
    assert os.path.isfile(
        os.path.join(output, "dummy_conda_forge_test-1.0-on-centos5-64", "meta.yaml")
    )
    with open(
        os.path.join(
            output,
            "dummy_conda_forge_test-1.0-on-centos5-64/",
...
def test_compute_builds_intradependencies(testing_workdir, monkeypatch, mocker):
    ...
    uses_zlib_job = [
        job for job in plan["jobs"] if job["name"] == "uses_zlib-1.0-on-centos5-64"
    ][0]
    assert any(
        task.get("passed") == ["zlib_wannabe-1.2.8-on-centos5-64"]
...
def test_subpackage_matrix_no_subpackages(monkeypatch):
    ...
    assert (
        ...
        "has_subpackages_subpackage-1.0-on-centos5-64",
    ) not in tasks.edges()
    # this is what we remap it to
    assert (
        "depends_on_subpackage-1.0-on-centos5-64",
        "has_subpackages_toplevel-1.0-on-centos5-64",
    ) in tasks.edges()
...
def test_dependency_with_selector_cross_compile(testing_conda_resolve):
    g = execute.collect_tasks(
        test_data_dir,
        ["selector_run", "functools32-feedstock"],
    ...
    # native edge
    assert (
        "test_run_deps_with_selector-1.0-python_2.7-on-win-64",
        "functools32_wannabe-3.2.3.2-python_2.7-on-win-64",
    ) in g.edges()
    # cross edge
    assert (
        "test_run_deps_with_selector-1.0-python_2.7-target_win-32-on-win-64",
        "functools32_wannabe-3.2.3.2-python_2.7-target_win-32-on-win-64",
    ) in g.edges()
...
def test_collapse_with_win_matrix_and_subpackages(monkeypatch):
    ...
    assert (
        "postgresql-split-10.1-c_compiler_vs2008-target_win-32-on-win-64"
        in tasks.nodes()
    )
    assert (
        "postgresql-split-10.1-c_compiler_vs2015-target_win-32-on-win-64"
        in tasks.nodes()
    )
...
def test_collapse_noarch_python():
    ...
    variant_file = os.path.join(
        test_data_dir, "noarch_python_recipes", "conda_build_config.yaml"
    )
    tasks = execute.collect_tasks(
        path,
        folders,

...
def testing_submodule_commit(testing_submodules_repo):
    ...
    subprocess.check_call(
        ["git", "checkout", "4648194ca029603b90de22e16b59949b5f68d2d5"]
    )
...
def testing_new_submodules(testing_submodules_repo):
    ...
    subprocess.check_call(
        ["git", "submodule", "add", "https://github.com/ContinuumIO/docker-images.git"]
    )
...
def testing_conda_resolve(request):
    ...
    if True:  # check for conda >= 4.3
        index = {
            conda_interface.Dist(
                dist_name="-".join((pkg, "920", "h68c14d1_0")),
                channel=None,
                name=pkg,
                version="920",
                build_string="h68c14d1_0",
                build_number=0,
            ): conda_interface.IndexRecord(
                arch="x86_64",
                build="h68c14d1_0",
                build_number=0,
                depends=tuple(),
                ...
                name=pkg,
                platform=conda_interface.cc_platform,
                requires=tuple(),
                size=192170,
                subdir=conda_interface.subdir,
                version="920",
                fn=pkg + "-920-h68c14d1_0.tar.bz2",
                schannel="r",
                channel="https://conda.anaconda.org/r/" + conda_interface.subdir,
                priority=1,
                url=(
                    "https://conda.anaconda.org/r/{}/pkg-920-h68c14d1_0.tar.bz2".format(
                        conda_interface.subdir, pkg
                    )
                ),
            )
            for pkg in pkgs

...
def test_get_run_test_deps(testing_metadata):
    assert compute_build_graph.get_run_test_deps(testing_metadata) == {
        "run_requirement": ("1.0", "any"),
        "test_requirement": ("any", "any"),
    }
...
def test_construct_graph_relative_path(testing_git_repo, testing_conda_resolve):
    ...
    assert set(g.nodes()) == {
        "test_dir_3-1.0-on-linux",
        "test_dir_2-1.0-on-linux",
...
def test_package_key(testing_metadata):
    assert (
        compute_build_graph.package_key(testing_metadata, "linux")
        == "test_package_key-1.0-python_3.6-on-linux"
    )
    assert (
        compute_build_graph.package_key(testing_metadata, "linux", "test")
...
def test_platform_specific_graph(mocker, testing_conda_resolve):
    ...
    mocker.patch.object(
        compute_build_graph,
        "_buildable",
        lambda name, version, recipes_dir, worker, config, finalize: os.path.join(
            recipes_dir, name
        ),
    )
...
def test_run_test_graph(testing_conda_resolve):
    ...
    assert set(g.nodes()) == set(
        ["c3itest-a-1.0-on-linux", "c3itest-b-1.0-on-linux", "c3itest-c-1.0-on-linux"]
    )
...
def test_add_dependency_nodes_and_edges(mocker, testing_graph, testing_conda_resolve):
    mocker.patch.object(compute_build_graph, "_installable")
    compute_build_graph._installable.return_value = False
    compute_build_graph.add_dependency_nodes_and_edges(
        "b-on-linux",
        testing_graph,
        run="build",
        worker=dummy_worker,
    ...
    assert set(testing_graph.nodes()) == {
        "a-on-linux",
        "b-on-linux",
        "c3itest-c-on-linux",
    }
...
def test_buildable(monkeypatch, testing_metadata):
    ...
    assert compute_build_graph._buildable(
        "somepackage", "any", test_data_dir, dummy_worker, config, False
    )
    assert compute_build_graph._buildable(
        "somepackage", "1.2.8", test_data_dir, dummy_worker, config, False
    )
    assert compute_build_graph._buildable(
        "somepackage", "1.2.*", test_data_dir, dummy_worker, config, False
    )
    assert compute_build_graph._buildable(
        "somepackage", ">=1.2", test_data_dir, dummy_worker, config, False
    )
    assert compute_build_graph._buildable(
        "somepackage", "<2", test_data_dir, dummy_worker, config, False
    )
    assert compute_build_graph._buildable(
        "somepackage", ">=1.2,<2", test_data_dir, dummy_worker, config, False
    )
    assert not compute_build_graph._buildable(
        "somepackage", "5", test_data_dir, dummy_worker, config, False
    )
    assert not compute_build_graph._buildable(
        "not_a_package", "any", test_data_dir, dummy_worker, config, False
    )


def test_installable(testing_conda_resolve, testing_metadata):
    assert compute_build_graph._installable(
        "a", "920", "any", testing_metadata.config, testing_conda_resolve
    )
    assert compute_build_graph._installable(
        "a", "920*", "any", testing_metadata.config, testing_conda_resolve
    )
    ...
    assert not compute_build_graph._installable(
        "a", "921", "any", testing_metadata.config, testing_conda_resolve
    )

    # default build number is 0
    assert compute_build_graph._installable(
        "a", "920", "h68c14d1_0", testing_metadata.config, testing_conda_resolve
    )
    assert not compute_build_graph._installable(
        "a", "920", "h68c14d1_1", testing_metadata.config, testing_conda_resolve
    )

    # package not in index
    assert not compute_build_graph._installable(
        "f", "any", "any", testing_metadata.config, testing_conda_resolve
    )
...
def test_expand_run_no_up_or_down(mocker, testing_graph, testing_conda_resolve):
    ...
    compute_build_graph.expand_run(
        testing_graph, Config(), testing_conda_resolve, worker=dummy_worker, run="build"
    )
...
def test_add_invalid_dir_to_graph(testing_graph, testing_conda_resolve):
    assert not compute_build_graph.add_recipe_to_graph(
...
def test_add_skipped_recipe(testing_graph, testing_conda_resolve):
    assert not compute_build_graph.add_recipe_to_graph(
        os.path.join(test_config_dir, "skipped_recipe"),
        testing_graph,
        run="build",
        worker=default_worker,
        conda_resolve=testing_conda_resolve,
...
def test_add_noarch_python_recipe(testing_conda_resolve):
    ...
    worker = default_worker
    conda_resolve = testing_conda_resolve
    compute_build_graph.add_recipe_to_graph(
        pkg_a_dir, graph, run, worker, conda_resolve
    )
    compute_build_graph.add_recipe_to_graph(
        pkg_b_dir, graph, run, worker, conda_resolve
    )
    assert "pkg_a-1.0.0-on-linux" in graph.nodes()
    assert "pkg_b-1.0.0-on-linux" in graph.nodes()
    assert graph.nodes["pkg_a-1.0.0-on-linux"]["noarch_pkg"] == True
    assert graph.nodes["pkg_b-1.0.0-on-linux"]["noarch_pkg"] == False
...
def test_add_intradependencies():
    ...
    b_meta = MetaData.fromdict(
        {"package": {"name": "b", "version": "1.0"}, "requirements": {"build": ["a"]}}
    )
...
def test_version_matching(testing_conda_resolve):
    ...
    assert (
        "downstream-1.0-upstream_1.0-on-linux",
        "upstream-1.0.1-on-linux",
    ) in g.edges()
    assert (
        "downstream-1.0-upstream_2.0-on-linux",
        "upstream-2.0.2-on-linux",
    ) in g.edges()

...
def test_upload_anaconda_with_user_and_label():
    ...
    assert {"--user", "llama", "--label", "dev"}.issubset(
        set(tasks[0]["config"]["run"]["args"])
    )
...
def test_upload_command_string():
    with open(os.path.join(test_config_dir, "config.yml")) as f:
        config_vars = yaml.safe_load(f)
    tasks = uploads.upload_commands(
        package_path="steve", commands="abc {package}", config_vars=config_vars
    )
...
def test_upload_command_list():
    with open(os.path.join(test_config_dir, "config.yml")) as f:
        config_vars = yaml.safe_load(f)
    tasks = uploads.upload_commands(
        package_path="steve", commands=["abc {package}", "wee"], config_vars=config_vars
    )
    assert tasks[0]["config"]["run"]["path"] == "abc"
    assert tasks[0]["config"]["run"]["args"] == ["steve"]
...
def test_get_upload_tasks(mocker, testing_graph):
    ...
    uploads.load_yaml_config_dir.return_value = [
        {"token": "abc"},
        {"server": "localhost"},
        {"commands": "weee"},
    ]

...
def parse_args(parse_this=None):
    ...
    examine_parser.add_argument(
        "base_name", help="name of your project, to distinguish it from other projects"
    )
    examine_parser.add_argument(
        "path",
        default=".",
    ...
    examine_parser.add_argument(
        ...
        type=int,
        help=(
            "Number of downstream steps to follow in the DAG when "
    ...
    examine_parser.add_argument(
        ...
        default="HEAD",
        help=(
            "start revision to examine.  If stop not "
    ...
    examine_parser.add_argument(
        ...
        default=None,
        help=(
            "stop revision to examine.  When provided," "changes are git_rev..stop_rev"
        ),
    )
    examine_parser.add_argument(
        "--test",
    ...
    bootstrap_parser = sp.add_parser(
        "bootstrap", help="create default configuration files to help you start"
    )
    ...
    one_off_parser.add_argument(
        "folders",
        nargs="+",
        help=("Specify folders, relative to --recipe-root-dir, to upload " "and build"),
    )
    one_off_parser.add_argument(
        "--automated-pipeline",
        action="store_true",
    ...
    one_off_parser.add_argument(
        "--pr-file", action="store", help="File added to the git repo by the PR"
    )
    one_off_parser.add_argument(
        "--stage-for-upload",
        action="store_true",
    ...
    one_off_parser.add_argument(
        ...
        action="store",
        help=(
            "git commit message to record when packages are uploaded, "
    ...
    one_off_parser.add_argument(
        "--output-dir",
        help=(
            "folder where output plan and recipes live."
    ...
    batch_parser.add_argument(
        ...
        type=int,
        help=(
            "maximum number of activate builds allowed before starting a new"
            "job, default is 6"
        ),
    )
    batch_parser.add_argument(
        ...
        type=int,
        help=(
            "time in seconds between checking concourse server for active "
    ...
    rm_parser.add_argument(
        "pipeline_names", nargs="+", help=("Specify pipeline names on server to remove")
    )
    rm_parser.add_argument(
        "--config-root-dir",
        help="path containing config.yml and matrix definitions",
    ...
    pause_parser.add_argument(
        "pipeline_names", nargs="+", help=("Specify pipeline names on server to pause")
    )
    pause_parser.add_argument(
        "--config-root-dir",
        help="path containing config.yml and matrix definitions",
    ...
    unpause_parser.add_argument(
        "--do-it-dammit", "-y", help="YOLO", action="store_true"
    )

    trigger_parser = sp.add_parser(
        "trigger", help="trigger (failed) jobs of a pipeline"
    )
    trigger_parser.add_argument(
        "pipeline_names", nargs="+", help=("Specify pipeline names to trigger")
    )
    trigger_parser.add_argument(
        "--config-root-dir",
        help="path containing config.yml and matrix definitions",
    ...
    trigger_parser.add_argument(
        "--all", dest="trigger_all", action="store_true", help="trigger all jobs"
    )

    abort_parser = sp.add_parser("abort", help="abort jobs of a pipeline")
    abort_parser.add_argument(
        "pipeline_names", nargs="+", help=("Specify pipeline names to abort")
    )
    abort_parser.add_argument(
        "--config-root-dir",
        help="path containing config.yml and matrix definitions",
...
def main(args=None):
    ...
    if args.subparser_name == "submit":
        ...
        raise NotImplementedError(
            "Command {} is not implemented".format(args.subparser_name)
        )

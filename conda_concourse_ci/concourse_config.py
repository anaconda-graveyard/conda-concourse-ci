...
class PipelineConfig:
        ...
        self.add_resource(
            ...
            source={
                ...
                "base_dir": os.path.join(
                    config_vars["intermediate-base-folder"], "source"
                ),
                "user": config_vars["intermediate-user"],
                "private_key": config_vars["intermediate-private-key-job"],
                "disable_version_path": True,
    ...
    def add_rsync_stats(self, config_vars):
        self.add_resource(
            ...
            source={
                "server": config_vars["intermediate-server"],
                "base_dir": os.path.join(
                    config_vars["intermediate-base-folder"], "stats"
                ),
                "user": config_vars["intermediate-user"],
                "private_key": config_vars["intermediate-private-key-job"],
                "disable_version_path": True,
    ...
    def add_anaconda_upload(self, all_rsync, config_vars):
        ...
        if config_vars.get("docker-user", None) and config_vars.get(
            "docker-pass", None
        ):
            _source.update(
                {
    ...
    def add_repo_v6_upload(self, all_rsync, config_vars):
        self.add_job(name="repo_v6_upload", plan=all_rsync + [{"put": "repo_resource"}])
        _source = {"repository": "condatest/repo_cli", "tag": "latest"}
        if config_vars.get("docker-user", None) and config_vars.get(
            "docker-pass", None
        ):
            _source.update(
    ...
    def add_push_branch_job(
        self, config_vars, folders, branches, pr_num, pr_merged_resource, stage_job_name
    ):
        plan = []
        if pr_merged_resource:
    ...
    def add_destroy_pipeline_job(self, config_vars, folders):
        ...
        config["params"] = params
        plan = [
            {"get": "pr-merged", "trigger": True, "passed": passed_jobs},
            {"task": "destroy-pipeline", "trigger": False, "config": config},
        ]
...
class JobConfig:
    ...
    def add_rsync_recipes(self):
        self.plan.append({"get": "rsync-recipes", "trigger": True})
    ...
    def add_rsync_prereq(self, prereq):
        self.plan.append(
            {"get": "rsync_" + prereq, "trigger": False, "passed": [prereq]}
        )

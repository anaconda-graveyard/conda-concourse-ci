...
def _git_changed_files(git_rev, stop_rev=None, git_root=""):
    ...
    output = subprocess.check_output(
        ["git", "diff-tree", "--no-commit-id", "--name-only", "-r", git_rev],
        cwd=git_root,
    )
...
def _get_or_render_metadata(meta_file_or_recipe_dir, worker, finalize, config=None):
    ...
    if (meta_file_or_recipe_dir, label, platform, arch) not in _rendered_recipes:
        ...
        _rendered_recipes[
            (meta_file_or_recipe_dir, label, platform, arch)
        ] = api.render(
...
def add_recipe_to_graph(
    ...
    try:
        ...
        log.warn(
            "invalid recipe dir or other recipe issue: %s - skipping.  Error was %s",
            recipe_dir,
            e,
        )
        return None
    ...
    for metadata, _, _ in rendered:
        if (
            is_package_built(metadata, "host", include_local=False)
            and config.skip_existing
            or metadata.skip()
        ):
            continue
    ...
    for node in graph.nodes():
        ...
        deps = set(
            ...
            + [
                ...
                for dep in ensure_list((m.meta.get("test") or {}).get("requires"))
            ]
        )
        ...
        for dep in deps:
            ...
            for matching_node in name_matches:
                ...
                if (
                    match_peer_job(conda_interface.MatchSpec(dep), match_meta, m)
                    ...
                    if all(
                        match_meta.config.variant[v] == m.config.variant[v]
        ...
        if "meta" in graph.nodes[node]:
            ...
            if master:
                if "master" in subgroup:
                    print(
                        f'tried to set {node} as master but {subgroup.get("master")} already is master.'
                    )
                    continue
                    raise ValueError(
...
def _write_recipe_log(path):
    ...
    try:
        ...
        log.warn("Unable to produce recipe git log for %s. Error was: %s", path, e)
        pass
...
def add_dependency_nodes_and_edges(
    node, graph, run, worker, conda_resolve, recipes_dir=None, finalize=False
):

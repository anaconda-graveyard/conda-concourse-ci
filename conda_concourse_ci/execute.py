...
def collect_tasks(
    ...
    for platform in platforms:
        ...
        conda_resolve = Resolve(
            get_build_index(
                subdir=subdir, bldpkgs_dir=config.bldpkgs_dir, channel_urls=channels
            )[0]
        )
...
def collapse_noarch_python_nodes(graph):
    ...
    for pkg_name, nodes in noarch_groups.items():
        ...
        if len(build_nodes) == 0:
            raise ValueError(
                "The %s platform has no noarch python build for %s"
                % (build_subdir, pkg_name)
            )
...
def get_build_task(
    ...
    worker_tags = ensure_list(worker_tags) + ensure_list(
        meta.meta.get("extra", {}).get("worker_tags")
    )
...
def graph_to_plan_with_jobs(
    ...
    base_folder = os.path.join(
        config_vars["intermediate-base-folder"], config_vars["base-name"]
    )
    ...
    for node in order:
        ...
        if automated_pipeline:
            ...
            jobconfig.plan.append({"get": pull_recipes_resource, "trigger": True})
        else:
            pull_recipes_resource = None
        ...
        if prereqs:
            jobconfig.add_consolidate_task(
                prereqs,
    ...
    if config_vars.get("anaconda-upload-token") or config_vars.get("repo-username"):
        all_rsync = [
            ...
            for node in order
            if graph.nodes[node]["worker"].get("rsync") is None
            or graph.nodes[node]["worker"].get("rsync") is True
        ]
        ...
        for build in builds:
            try:
                curtime = datetime.fromtimestamp(
                    build.get("end_time", build.get("start_time"))
                )
...
def submit(
    ...
    **kw,
    ...
    pipeline_name = pipeline_name.format(
        base_name=base_name, git_identifier=git_identifier
    )
    ...
    # this is a one-off job.  Sync the recipes we've computed locally.
        ...
        subprocess.check_call(
            [
                ...
                (
                    "{intermediate-user}@{intermediate-server}:"
                    "{intermediate-base-folder}/{base-name}/plan_and_recipes".format(
                        **data
                    )
                ),
            ]
        ...
        subprocess.check_call(
            [
                ...
                key_file,
                "{intermediate-user}@{intermediate-server}".format(**data),
                "rm -rf {intermediate-base-folder}/{base-name}/artifacts".format(
                    **data
                ),
            ]
        )
    ...
    **kw,
    ...
    if kw.get("stage_for_upload", False):
        if kw.get("commit_msg") is None:
            raise ValueError("--stage-for-upload requires --commit-msg to be specified")
    ...
    for var_val in build_config:
        if var_val not in matched:
            ...
            if "*" in var:
                log.warning(
                    "Did not find match for --build-config={} (it has no effect)".format(
                        var_val
                    )
                )
            else:
    ...
    for node in nodes:
        ...
        assert recipe, (
            "no parent recipe set, and no path associated " "with this metadata"
        )
        ...
        if clobber_sections_file:
            shutil.copyfile(
                clobber_sections_file, os.path.join(out_folder, "recipe_clobber.yaml")
            )

        if append_sections_file:
            shutil.copyfile(
                append_sections_file, os.path.join(out_folder, "recipe_append.yaml")
            )
        ...
        if not last_recipe_dir or last_recipe_dir != recipe_dir:
            order_recipes_fn = (
                "output_order_recipes_" + task_graph.nodes[node]["worker"]["label"]
            )
...
def bootstrap(base_name, pass_throughs=None, **kw):
    ...
    for run_type in ("build", "test"):
        if not os.path.exists("{0}/{1}_platforms.d".format(base_name, run_type)):
            _copy_yaml_if_not_there(
                "{0}/{1}_platforms.d/example.yml".format(base_name, run_type), base_name
            )
    if not os.path.exists("{0}/uploads.d".format(base_name)):
        _copy_yaml_if_not_there(
            "{0}/uploads.d/anaconda-example.yml".format(base_name), base_name
        )
        _copy_yaml_if_not_there(
            "{0}/uploads.d/scp-example.yml".format(base_name), base_name
        )
        _copy_yaml_if_not_there(
            "{0}/uploads.d/custom-example.yml".format(base_name), base_name
        )
    # create initial plan that runs c3i to determine further plans
    #    This one is safe to overwrite, as it is dynamically generated.
    shutil.copyfile(
        os.path.join(bootstrap_path, "plan_director.yml"), "plan_director.yml"
    )
    # advise user on what to edit and how to submit this job
    print(
        """ Wrote bootstrap config files into '{0}' folder.
...
def submit_one_off(
    ...
    **kwargs,
    ...
    ctx = (
        contextlib.contextmanager(lambda: (yield kwargs.get("output_dir")))
    ...
    with ctx() as tmpdir:
        ...
        compute_builds(
            path=recipe_root_dir,
            base_name=pipeline_label,
            folders=folders,
            matrix_base_dir=config_root_dir,
        ...
        if kwargs.get("dry_run", False):
            ...
            submit(
                pipeline_file=os.path.join(tmpdir, "plan.yml"),
                base_name=pipeline_label,
...
def rm_pipeline(
    ...
    **kwargs,
    ...
    if days:
        pipelines_to_remove = _filter_pipelines_by_time(
            con, pipelines_to_remove, int(days)
        )
...
def pause_pipeline(
    pipeline_names, config_root_dir, do_it_dammit=False, pass_throughs=None, **kwargs
):
    con = _ensure_login_and_sync(config_root_dir)
...
def trigger_pipeline(pipeline_names, config_root_dir, trigger_all=False, **kwargs):
    ...
    for pipeline in pipelines_to_trigger:
        for job in con.get_jobs(pipeline):
            ...
            if any(
                sub in job.get("name")
                for sub in ["stage_for_upload", "push_branch_to", "destroy_pipeline"]
            ):
                continue
            if status != "succeeded":
                print(f"{pipeline}/{job['name']}")

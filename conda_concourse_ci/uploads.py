...
def upload_anaconda(package_path, config_vars, token, user=None, label=None):
    ...
    task = _base_task(
        upload_job_name,
        config_vars.get("docker-user", None),
        config_vars.get("docker-pass", None),
    )
...
def upload_scp(
    package_path, server, destination_path, auth_dict, worker, config_vars, port=22
):
    """
    ...
    index_args = [
        ...
        port,
        server,
        "conda index {0}".format(destination_path),
    ]
...
def get_upload_tasks(
    graph, node, upload_config_path, config_vars, commit_id, public=True
):
    upload_tasks = []
    ...
    for package in api.get_output_file_paths(meta):
        ...
        package_path = os.path.join(
            "output-artifacts", commit_id, meta.config.host_subdir, filename
        )
        for config in configurations:
            if "token" in config:
                tasks = upload_anaconda(
                    package_path=package_path, config_vars=config_vars, **config
                )
                ...
                tasks = upload_commands(package_path, config_vars=config_vars, **config)
            else:
                raise ValueError(
                    "Unrecognized upload configuration.  Each file needs one of: "
